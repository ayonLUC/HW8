...
//        return -1;
    ...
    private void dfsFromMap(int node, Map<Integer, List<Integer>> graph, boolean[] visited) {
        visited[node] = true;
        if (!graph.containsKey(node)) return; // No neighbors

        for (int neighbor : graph.get(node)) {
            if (!visited[neighbor]) {
                dfsFromMap(neighbor, graph, visited);
            }
        }
    }
